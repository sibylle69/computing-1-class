...
class ImperialStudent():
    ...
    def get_next_due(self):
        min_date = self.on_loan[list(self.on_loan.keys())[0]]
        next_item = list(self.on_loan.keys())[0]
        for key in self.on_loan:
            if self.on_loan[key] < min_date:
                next_item = key
        return next_item
...
class DEStudent(ImperialStudent):
    ...
    def __init__(self, surname, year):
        self.year = year 
        ImperialStudent.__init__(self,surname)
